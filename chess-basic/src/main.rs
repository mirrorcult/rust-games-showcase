...
impl Chessboard {
  ...
  fn is_legal_move(&self, start: (usize, usize), end: (usize, usize)) -> bool {
    ...
    match self.pieces[self.rowcol_to_idx((sr, sc))] {
      ...
      Rook(_) => {
        (sr == er && sc != sc) || (sc == ec && sr != er)
      },
      Bishop(_) => {
        (ec as isize - sc as isize).abs() == (er as isize - sr as isize).abs()
      },
      _ => true
    }
  }
...
fn main() {
  ...
  loop { 
    ...
    let start_rowcol = (start_iter.next().unwrap().parse().unwrap(), start_iter.next().unwrap().parse().unwrap());
    let end_rowcol = (end_iter.next().unwrap().parse().unwrap(), end_iter.next().unwrap().parse().unwrap());
